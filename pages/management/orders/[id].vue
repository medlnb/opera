...
<template>
  <div>
    ...
    <!-- Not Found State -->
    <VCard v-else-if="!order" class="text-center py-12">
      <VIcon icon="tabler-package-off" size="64" class="text-disabled mb-4" />
      <h3 class="text-h6 text-disabled mb-2">Order not found</h3>
      <p class="text-body-2 text-disabled mb-4">The order you're looking for doesn't exist.</p>
      <VBtn color="primary" to="/management/orders">
        <VIcon icon="tabler-arrow-left" class="me-2" />
        Back to Orders
      </VBtn>
    </VCard>
      ...
      <VRow>
        ...
        <VCol cols="12" md="8">
          <VCard class="mb-6">
            ...
            <VCardText>
              ...
              <!-- Order Items -->
              <h4 class="text-h6 mb-4">Order Items</h4>
              <VTable>
                <thead>
                  <tr>
                    <th>Product</th>
                ...
                <tbody>
                  <tr v-for="(item, index) in order.items" :key="index">
                    <td>
                      <div class="font-weight-medium">{{ item.title || 'Unknown Product' }}</div>
                    </td>
                    <td>{{ item.quantity || '-' }}</td>
                    <td>{{ item.color || '-' }}</td>
                    <td class="text-center">{{ item.qty }}</td>
                    <td class="text-end font-weight-medium">{{ (item.price * item.qty).toLocaleString() }} DZD</td>
                  </tr>
                </tbody>
                <tfoot>
                  <tr>
                    <td colspan="4" class="text-end font-weight-bold">Total:</td>
                    <td class="text-end font-weight-bold text-primary">{{ order.total?.toLocaleString() }} DZD</td>
                  </tr>
                </tfoot>
              </VTable>
            </VCardText>
        ...
        <VCol cols="12" md="4">
          ...
          <VCard class="mb-6">
            ...
            <VDivider />
            <VCardText>
              <div v-if="order.user" class="d-flex flex-column gap-3">
                <div>
            ...
            <VCardText>
              <VTimeline side="end" density="compact">
                ...
                >
                  ...
                  <div class="text-body-2">{{ formatDate(order.createdAt) }}</div>
                </VTimelineItem>
                <VTimelineItem
