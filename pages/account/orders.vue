...
<template>
  <div>
    <VCard title="My Orders" class="mb-6">
      <VCardText>
        <VDivider class="my-4" />
        ...
        >
          ...
          <template #item.status="{ item }">
            <VChip label size="small" color="info" variant="tonal">{{ item.status }}</VChip>
          </template>
          ...
          <template #no-data>
            <div class="text-center py-12">
              <VIcon icon="tabler-receipt-off" size="64" class="text-disabled mb-4" />
              <p class="text-h6 text-disabled">No orders yet</p>
              <p class="text-body-2 text-disabled">Your order history will appear here</p>
              <VBtn color="primary" to="/products/interior" class="mt-4">Start Shopping</VBtn>
            </div>
          </template>
